...
export const useAdminStore = defineStore("admin", {
  ...
  actions: {
    async getAllUsersWithActivity(filters = {}) {
      ...
      this.loading = true
      try {
        const response = await api.get("/admin/users-with-activity", { params: filters })
        this.users = response.data.users
        return response.data
      } catch (error) {
        console.error("Error fetching users:", error)
        this.error = error.response?.data?.message || "Error loading users"
        throw error
      } finally {
        this.loading = false
      }
    ...
    async updateUserRole(userId, role) {
      ...
      try {
        ...
        const updatedUserIndex = this.users.findIndex(u => u._id === userId)
        ...
        useToast().error(error.response?.data?.message || "Error updating role")
        throw error
      } finally {
        this.loading = false
      }
    ...
    async createUser(userData) {
      ...
      try {
        ...
        useToast().error(error.response?.data?.message || "Error creating user")
        throw error
      } finally {
        this.loading = false
      }
    ...
    async updateUser(userId, userData) {
      ...
      try {
        ...
        useToast().error(error.response?.data?.message || "Error updating user")
        throw error
      } finally {
        this.loading = false
      }
    ...
    async deleteUser(userId) {
      ...
      try {
        ...
        useToast().error(error.response?.data?.message || "Error deleting user")
        throw error
      } finally {
        this.loading = false
      }
    ...
    async updateOrderStatus(orderId, status) {
      ...
      try {
        ...
        useToast().error(error.response?.data?.message || "Error updating status")
        throw error
      } finally {
        this.loading = false
      }
    ...
    async getDashboardStats() {
      ...
      try {
        ...
        this.error = error.response?.data?.message || "Error loading stats"
        throw error
      } finally {
        this.loading = false
      }
